...
#include "alut.h"
int runArcan(LedKeyboard &kbd, std::string arg1) {
	int ch;
	uint8_t buf = 0, led = 0;

	struct LedKeyboard::KeyValue kv;
	std::vector<LedKeyboard::KeyValue> keybuffer = {};

	bool all = false;

	FILE* fin = arg1 == "-" ? stdin : fopen(arg1.c_str(), "r");
	...
	while (EOF != (ch = fgetc(fin))){
		...
		case 'a': all = false; led = ch; break;
		case 'r': kv.color.red = ch; break;
		case 'g': kv.color.green = ch; break;
		case 'b': kv.color.blue = ch; break;
		case 'i': kv.color.red = kv.color.green = kv.color.blue = ch; break;
		case 'o': return 1; break;
		case 'c':
			if (all){
/*				std::cout << "set all" << std::endl; */
				kbd.setAllKeys(kv.color);
			}
			else if (led != 255){
				kv.key = alut[led];
/*				std::cout << "set led " << (int)led << std::endl; */
				keybuffer.push_back(kv);
			}

			if (ch == 0){
/*				std::cout << "commit" << std::endl; */
				if (keybuffer.size()){
					kbd.setKeys(keybuffer);
					keybuffer.clear();
				}
...
/*				std::cout << "buffer" << (int)ch << std::endl; */
			}
...
int main(int argc, char **argv) {
	...
	while (argIndex < argc)
	{
		...
		else if (argc > (argIndex + 1) && arg == "--aled") return runArcan(kbd, argv[argIndex + 1]);
